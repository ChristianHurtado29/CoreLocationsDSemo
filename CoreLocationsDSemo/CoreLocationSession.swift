...
class CoreLocationSession: NSObject {
    ...
    private func startSignificantLocationChanges() {
        ...
        // less aggressive than the startUpdatingLocation() in GPS monitor changes
        locationManager.startMonitoringSignificantLocationChanges()
    }
    
    public func convertCoordeinateToPlacemark(coordinate: CLLocationCoordinate2D) {
        // we will use the CLGeoder( class for converting coordinate(CLLocationCoordinate2D) to placemark (CLPlacemark)
    ...
    public func convertPlaceNameToCoordinate(addressString: String) {
        ...
        CLGeocoder().geocodeAddressString(addressString) { (placemarks, error) in
            ...
            if let firstPlacemark = placemarks?.first,
                ...
                print("\(addressString) is here...... \(location.coordinate)")
            }
        }
    }
